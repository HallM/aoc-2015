...
const commands = `turn off 660,55 through 986,197
...
// const commands = `turn off 0,0 through 999,999`;
// const commands = `toggle 0,0 through 999,999`;
...
function processCommand(cmd) {
  ...
  const firstParts = cmdParts[numStart].split(',');
  const x1 = parseInt(firstParts[0]);
  const y1 = parseInt(firstParts[1]);

  const secondParts = cmdParts[numStart + 2].split(',');
  const x2 = parseInt(secondParts[0]);
  const y2 = parseInt(secondParts[1]);
...
console.log(grid.reduce(function(count, line) {
  return line.reduce(function(count, item) {
    return count + item; // (item ? 1 : 0);
  }, count);
}, 0));
